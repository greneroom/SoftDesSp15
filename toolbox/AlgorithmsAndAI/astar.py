...
class GridWorld():
    ...
    def _init_cells(self):
        ...
        for i in range(self.height):
            for j in range(self.width):
                self.cells[(i, j)] = Cell(self.screen, (i * self.cell_size, j * self.cell_size),
    ...
    def _add_swamp(self, mouse_pos):
        ...
        if self._is_occupied(swamp_coord):
            ...
            self.actors[swamp_coord] = ObstacleTile(swamp_coord, self, './images/swamp.jpg', is_unpassable=False,
                                                    terrain_cost=3)
    ...
    def _add_lava(self, mouse_pos):
        ...
        if self._is_occupied(lava_coord):
            ...
            self.actors[lava_coord] = ObstacleTile(lava_coord, self, './images/lava.jpg', is_unpassable=True,
                                                   terrain_cost=0)
...
class Actor(object):
    ...
    def draw(self):
        ...
        x_y_coords = self.world._add_coords(cell.coordinates,
                                            (3, 3))  #add an offset so that the image will fit inside the cell border.
...
class ObstacleTile(Actor):
    def __init__(self, cell_coordinates, world, image_loc, terrain_cost=0, is_unpassable=True):
        super(ObstacleTile, self).__init__(cell_coordinates, world, image_loc, unremovable=False,
...
class Paul(Actor):
    ...
    def get_open_adj_coords(self, coords):
        ...
        in_bounds = [self.world._is_in_grid(c) and not self.world._is_occupied(c) and c not in self.closed_list for c in
                     adj_coords]
